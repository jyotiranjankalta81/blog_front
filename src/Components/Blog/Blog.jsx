...
const Blog = () => {
    ...
    const getAllBlogs = () => {
        axios.get('https://blog-back-n3dc.vercel.app/getblogs').then((res) => {
...

                    {
                        blogInfo?.map((values) => {
                            return (
                                ...
                                <div key={values._id}>
                                    <div onClick={() => handlepressReadmore(values)} className="h-56  rounded-sm px-5 py-5 space-y-5 hover:cursor-pointer hover:shadow-gray-600 border-b-2 hover:shadow-md hover:-translate-y-1 transition-all" >

                                        <div className='my-2'>
                                            <span className="category px-2 py-1  bg-indigo-800  text-white rounded-md">{values?.category}</span>
                                        </div>

                                        <div className='my-2'>
                                            <span className="category text-2xl text-white rounded-md">{values?.mainHeading}</span>
                                        </div>

                                        <p className='text-white my-5 text-sm '>
                                            {/* {values?.description} */}

                                            <div dangerouslySetInnerHTML={{ __html: values?.description?.slice(0, 100) }} /> <span>...</span>
                                        ...
                                        <div className="space-x-5">
                                            <Link className='text-violet-500 flex items-center space-x-5'>
                                                <p>Learn More</p> <span><BsArrowRight className='text-xl' /></span>
                                            </Link>
                                        </div>
                                    </div>
                                </div>
